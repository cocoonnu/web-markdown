...
- 首先将模块用 flex 按区域划分，使用百分比给每块分配占比
- 然后要保证内容显示完整，需要设置最小宽度和最小高度
- 使用上面两个规范就可以适配大屏幕和小屏幕了



整体框架设计：

- 已经规范了每个盒子的比例，也给盒子添加了默认的间距
- 定义了几个公用变量在最外层的样式文件中，主要是规范间距
...
新增档案号、档案盒号完全采用自定义配置规则：

- 组件位置：`src/plugins/archives-number/components/right/custom-form-item/DocumentSet.tsx`
- 默认显示规定的配置项，之后允许用户修改删除。选择的配置项不能重复（自定义字段除外）
- 流水号和案卷号要固定到最下面不能移动和删除

分析 DocumentSet 组件：新建档案号、档案盒号，该组件会得到一个初始化的值。具体赋值位置：`src/plugins/archives-number/vms/archives-number-manger.vm.ts`，聚焦在这个函数中：createInit



修复案卷号拖拽问题

- 案卷号新增字段规则为默认存在流水号，且不允许删除，但是可以移动
- 其他新增字段可以移动和删除，但是不能重复（自定义字段除外）



## PRD-拓展检测范围可选维度

高级设置 - 关系选择属于时，最右方的选择下拉框需要支持多选

- 组件位置：`src/plugins/common/components/Fields/FieldConditional/index.tsx`
- 修改代码位置：`src/plugins/common/components/Fields/FieldConditional/utils.ts/computeConditionRuleComponent`

在这个通用表单组件中分成三个区域：field 字段下拉选择、规则下拉选择、右侧动态组件

首先是选择了档案类型之后会得到该档案类型的 components，因此 field 字段下拉选项可以确定。选择 field 字段后规则下拉选项又可以确定了，最后可以确定右侧动态组件。**获取指定 fieldItem 的模板数据的函数：getFieldData。**
...
export class EkbCollection extends React.Component {
  @inject vm: EkbCollectionVM // 注入之后内层组件才被允许使用
...
export const Operation = (value, record) => {
  const vm = useInstance(EkbCollectionVM) // 实例化vm

  return useObserver(() => { // 往往需要useObserver语句嵌套才能访问vm里面的数据
...
在 VM 注入一个其他的 VM 方式如下（此 vm 一定是被实例化过的vm，比如LayoutVM，在layout层就已经被实例化）：
...
我们可以设置树节点属性 selectable 使该节点不可选，即点击该节点时不会触发 onChange 函数
...
设置 dropdownMatchSelectWidth 属性即可，`dropdownMatchSelectWidth: 360| true`

https://ant-design.antgroup.com/components/auto-complete-cn#api
